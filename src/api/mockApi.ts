...
const entidades: Entidade[] = [
  ...
  },
  {
    ...
    telefones: [
      { tipo: 'Comercial', numero: '(11) 4002-8922' },
    ],
...
export type TipoBusca = 'cpf' | 'cnpj' | 'email' | 'telefone' | 'endereco' | 'nome';

export async function buscarEntidades(tipo: TipoBusca, termo: string): Promise<Entidade[]> {
  ...
  return entidades.filter((entidade) => {
    switch (tipo) {
      ...
      case 'email':
        return entidade.emails.some((email) => email.toLowerCase().includes(termoLower));
      case 'telefone':
